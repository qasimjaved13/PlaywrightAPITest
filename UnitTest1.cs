...
using PlaywrightAPITest.Driver;
using Xunit;
...
public class UnitTest1 : IClassFixture<PlaywrightDriver>
{
    ...
    {
        var response = await _playwrightDriver.ApiRequestContext?.GetAsync("api/users/2");
    ...
    {
        var response = await _playwrightDriver.ApiRequestContext.PostAsync("api/users",
             new APIRequestContextOptions
             {
                 DataObject = new
                 {
